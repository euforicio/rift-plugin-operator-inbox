...
import { ArchiveIcon, ArrowClockwiseIcon, CaretDownIcon, FileIcon, EnvelopeOpenIcon, PaperPlaneTiltIcon } from "@phosphor-icons/react";
  ...
  return <div data-testid="message-body" className="min-w-0 break-words text-sm leading-6">
    <ReactMarkdown remarkPlugins={[remarkBreaks]} components={{
      ...
      a: ({ href = "", children }) => {
        ...
        return <button type="button" title="Open file preview" className="inline-flex max-w-full cursor-pointer items-center gap-1.5 rounded-md border border-border bg-muted/20 px-2 py-1 align-middle text-left text-primary hover:bg-muted/50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary" onClick={() => {
          setOpenError(!navigate.experimental_openFilePreview({ target, location: null }));
        }}><FileIcon aria-hidden="true" size={16} className="shrink-0" /><span className="min-w-0 break-words">{children}</span></button>;
      },
    }}>{text}</ReactMarkdown>
    ...
    {sidebar.projects.length === 0 ? <section className="rounded-lg border border-dashed border-border p-6 text-center"><h2 className="font-medium">No projects available</h2><p className="mt-1 text-sm text-muted-foreground">A project is required before operator messages can appear here.</p></section> : <section aria-label={`${currentProjectLabel} messages`} className="grid min-w-0 gap-3">
      ...
      <div role="list" aria-label="Operator messages" className="grid min-w-0 gap-3">{visibleMessages.map((message) => {
        ...
        return <article key={key} role="listitem" className={`min-w-0 overflow-hidden rounded-lg border bg-background ${selected ? "border-primary/50" : "border-border"}`}>
          <h2><button id={toggleId} type="button" aria-expanded={selected} aria-controls={detailId} aria-label={`${selected ? "Collapse" : "Expand"} message ${messageNumberLabel(message)} from ${sender}. ${projectNames.get(message.projectId) ?? message.projectId}. ${severityLabel(message.severity)}. ${stateLabel(message)}. ${formatExactTime(message.createdAtMs)}`} onClick={() => setSelectedMessageKey(selected ? null : key)} className="grid w-full min-w-0 gap-2 p-3 text-left hover:bg-muted/30 focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary">
            <span className="flex min-w-0 items-center gap-2">
              {message.readAtMs === null && <span aria-label="Unread" className="h-2 w-2 shrink-0 rounded-full bg-primary" />}
              <span className="min-w-0 flex-1 truncate text-sm font-semibold" title={sender}>{sender}</span>
              ...
              <CaretDownIcon aria-hidden="true" size={16} className={`shrink-0 text-muted-foreground ${selected ? "rotate-180" : ""}`} />
            </span>
          ...
          {!selected && message.archivedAtMs === null && <div className="flex justify-end px-3 pb-2"><button type="button" aria-label={pendingAction?.key === key && pendingAction.action === "archive" ? "Archiving message" : "Archive message"} title="Archive message" disabled={pendingAction !== null} className="flex min-h-8 min-w-8 items-center justify-center rounded-md text-muted-foreground hover:bg-muted hover:text-foreground focus-visible:outline focus-visible:outline-2 focus-visible:outline-primary disabled:opacity-50" onClick={() => { void archiveMessage(message).catch(() => undefined); }}><ArchiveIcon aria-hidden="true" weight="duotone" size={16} /></button></div>}
        </article>;
      })}</div>
    </section>}
  </div></main>;
